...
exports.getDrops = async (req, res) => {
  try {
    const drops = await prisma.drops.findMany();
    res.json(drops);
  } catch (err) {
...
exports.joinWaitlist = async (req, res) => {
  ...
  try {
    const count = await prisma.waitlists.count({ where: { drop_id: dropId } });
    const entry = await prisma.waitlists.create({
...
exports.leaveWaitlist = async (req, res) => {
  ...
  try {
    const deleted = await prisma.waitlists.deleteMany({
...
exports.claimDrop = async (req, res) => {
  ...
  try {
    // Waitlist kontrolü
    const entry = await prisma.waitlists.findFirst({
      where: { user_id: userId, drop_id: dropId },
    });
